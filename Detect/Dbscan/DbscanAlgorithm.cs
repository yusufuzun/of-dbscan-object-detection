...
namespace Detect.Dbscan
{
    ...
    {
        private readonly Func<T, T, double> metricFunc;
        private readonly Func<T, T, bool> extraPredicate;

        /// <summary>
        /// Takes metric function to compute distances between dataset items T
        /// </summary>
        /// <param name="metricFunc"></param>
        /// <param name="extraPredicate"></param>
        public DbscanAlgorithm(Func<T, T, double> metricFunc, Func<T, T, bool> extraPredicate)
        {
        ...
        /// <returns>returns sets of clusters, renew the parameter</returns>
        ...
        {
            neighborPts = allPoints.Where(x => metricFunc(point, x.ClusterPoint) <= epsilon * epsilon
                && (extraPredicate == null || extraPredicate(point, x.ClusterPoint))).ToArray();
        }
    }
}